...
## 2026-07-06 小程序 AI 图片卡片改为优先展示原图地址并在代理下载失败时自动回退

## 2026-07-06 AI 生成图片上传 COS 后同步写入文件管理记录并显示为 AI生成分组

## 2026-07-06 AI 对话返回首页增加整页跳转兜底，避免动态模块加载失败卡住

## 2026-07-06 AI 图片上传 COS 后返回预签名 URL，避免私有桶直链 403

## 2026-07-06 网页端 AI 图片预览改为直接使用 COS 地址并清除旧代理加载状态

## 2026-07-06 AI 图片生成支持小程序图片预览、尺寸选择和基于上一张图继续调整

## 2026-07-06 网页端 AI 图片结果支持代理预览、尺寸选择，SenseNova 调整改为视觉读图后再出图

## 2026-07-06 AI 图片生成结果改为下载后上传腾讯 COS 并返回稳定访问地址

## 2026-07-06 小程序首页 AI 助手入口从浮动按钮改为 header 内嵌按钮

- 移除底部浮动 AI 按钮（`ai-fab`），避免遮挡基金列表内容
- 在 header 右侧新增 AI 助手入口按钮（渐变背景 + 图标 + 文字）
- 优化页面布局，header 改为 flex 左右布局，左侧标题右侧 AI 入口
- 深色主题适配

## 2026-07-06 Dashboard AI 智能助手改为入口卡片，新增独立 AI 对话页面

- Dashboard 页面 AI 智能助手从内嵌对话框改为渐变入口卡片，点击跳转到 `/ai/chat` 新页面
- 新增 `views/ai/Chat.vue` 全屏 AI 对话页面，采用主流 AI 对话风格（类似 ChatGPT/Claude）
- 支持快捷操作卡片、消息列表、Markdown 渲染、复制/重新生成、清空历史等功能
- 路由新增 `/ai/chat`（独立顶级路由，不嵌套 Layout）
...
## 2026-07-07 新增 AI 作图专用页面与多图生成接口（不上传 COS/OSS，主页入口）

## 2026-07-08 修复商汤 AI 作图模型仅允许 n=1 时多图生成失败的问题（按数量拆分请求并聚合结果）

## 2026-07-08 商汤 AI 作图多图生成由顺序拆分请求改为线程池并发请求并按顺序聚合

## 2026-07-08 AI 作图结果图片按生成尺寸等比例展示，取消方图裁切

## 2026-07-08 AI 作图页面新增右侧抽屉提示词润色功能并提供专用后端润色接口

## 2026-07-08 小程序登录页新增非手动退出场景微信自动登录，失败后回退账号密码/微信一键登录

## 2026-07-09 小程序微信自动登录期间隐藏登录表单并展示独立加载态
